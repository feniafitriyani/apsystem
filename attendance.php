	...
	if(isset($_POST['employee'])){
		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					$output['error'] = true;
					$output['message'] = 'Sudah Melakukan Absen';
				}
				else{
					//updates
					...
					if($conn->query($sql)){
						$output['message'] = 'Silahkan Masuk: '.$row['firstname'].' '.$row['lastname'];
					}
					else{
						$output['error'] = true;
				...
				if($query->num_rows < 1){
					$output['error'] = true;
					$output['message'] = 'Tidak Dapat Keluar. Karena Belum Masuk';
				}
				else{
					$row = $query->fetch_assoc();
					if($row['time_out'] != '00:00:00'){
						$output['error'] = true;
						$output['message'] = 'Sudah Absen Keluar Hari ini';
					}
					else{
			...
			$output['error'] = true;
			$output['message'] = 'Siswa Tidak Dapat Ditemukan';
		}
